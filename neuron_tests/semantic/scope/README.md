...
| `NPP2001`, `NPP4001` | current coarse fallbacks |
...
| `scope__self_module_import_rule` | `module Box; Box is class { value is 0 as int; }` in `Box.npp` under strict naming rules | `N4000-N4099` or semantic fallback | Matches a current strict-naming semantic contract |
| `scope__class_filename_mismatch` | `Vec2 is class { x is 0 as int; }` in `Vector2.npp` under strict naming rules | `N4000-N4099` or semantic fallback | Ensures filename-based scope rules stay explicit |
