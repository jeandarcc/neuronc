...
This folder covers behavior exposed through standard modules when called from `.npp` source. The oracle is still user-observable source behavior, not implementation details of the runtime libraries.
...
- `System`, `IO`, `Math`, and collection/string-like facilities as observable from `.npp`
...
| `NPP2001`, `NPP4001`, `NPP0001` | current coarse fallbacks |
...
| `stdlib_baseline__missing_module` | `module MissingPkg; Init is method() {}` | `N4000-N4099` or `NPP4001` | Keeps source-level module errors in scope |
...
Happy-path stdlib tests should show the exact visible effect of the API from `.npp` source, not only that the call type-checks.
