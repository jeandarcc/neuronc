...
This folder covers diagnostics for module declarations, imports, file/module naming contracts, and modulecpp integration as seen from `.nr` source.
...
| `diag_module__self_import_rule` | `module Box; Box is class {}` in `Box.nr` with strict naming | Message states module/file cannot import itself | Existing repo-specific rule should stay understandable |
| `diag_module__class_filename_mismatch` | `Vec2 is class {}` in `Vector2.nr` with strict naming | Message mentions both class name and filename mismatch | Critical for project-structure usability |
