...
| Script | Purpose |
|--------|---------|
| `build.bat` | The canonical full-build script. Generates the `build-mingw` directory in the UX-defined workspace cache (`%LOCALAPPDATA%\NeuronPP\workspaces\`), runs CMake, triggers Ninja, and copies all necessary runtime DLLs. |
...
| `strip_utf8_bom.py` | A maintenance utility to recursively strip Byte-Order Marks from source files (which break certain older LLVM tools if they leak out of the NPP frontend). |
