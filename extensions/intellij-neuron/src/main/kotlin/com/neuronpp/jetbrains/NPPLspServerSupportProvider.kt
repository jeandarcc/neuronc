...
class NeuronLspServerSupportProvider : LspServerSupportProvider {
    override fun fileOpened(project: Project, file: VirtualFile, serverStarter: LspServerSupportProvider.LspServerStarter) {
        if (file.fileType == NeuronFileType) {
            val executable = resolveLspExecutable(project)
            if (executable != null) {
                serverStarter.ensureServerStarted(NeuronLspServerDescriptor(project, executable))
            }
        }
    }
...
class NeuronLspServerDescriptor(project: Project, private val executablePath: String) : ProjectWideLspServerDescriptor(project, "Neuron") {
    override fun isSupportedFile(file: VirtualFile) = file.fileType == NeuronFileType
