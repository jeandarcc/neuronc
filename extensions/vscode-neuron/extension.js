...
function isNppDocument(document) {
  // Only accept canonical 'neuron' language id; legacy 'npp' fallback has been removed
  return document && document.languageId === "neuron";
}
...
class NeuronLspClient {
  ...
  resolveServerCommand() {
    ...
    for (const folder of folders) {
      ...
      // Also check the canonical Neuron workspace state root
      if (localAppData) {
        candidates.push(
          path.join(localAppData, "Neuron", "workspaces", folderName, "build-mingw", "bin", "neuron-lsp.exe"),
          path.join(localAppData, "Neuron", "workspaces", folderName, "build-mingw", "bin", "neuron-lsp"),
          path.join(localAppData, "Neuron", "workspaces", folderName, "build", "bin", "neuron-lsp.exe"),
          path.join(localAppData, "Neuron", "workspaces", folderName, "build", "bin", "neuron-lsp"),
        );
      }
  ...
  installEditorBindings() {
    ...
    this.hoverProvider = vscode.languages.registerHoverProvider("neuron", {
    ...
    this.renameProvider = vscode.languages.registerRenameProvider("neuron", {
    ...
    this.inlayHintsProvider = vscode.languages.registerInlayHintsProvider("neuron", {
    ...
    this.codeLensProvider = vscode.languages.registerCodeLensProvider("neuron", {
